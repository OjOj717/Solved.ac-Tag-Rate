...
module.exports = async (req, res) => {
    ...
    try {
        // 2. API 데이터 병렬 호출 (태그 정보 + 유저 전체 정보)
        ...
        const userSolvedCount = userRes.data?.solvedCount || 1; // 0 나누기 방지
        if (allTags.length === 0) return res.status(404).send('No tag data found.');

        // 3. 레이팅 높은 순으로 정렬 후 상위 N개 추출
        ...
        const col2X = width - 145; // '문제' 컬럼 중심 좌표
        const col3X = width - 55;  // '레이팅' 컬럼 중심 좌표
